...
func (s *Server) handleUpdateRepo(w http.ResponseWriter, r *http.Request) {
	...
	// Only overwrite the SSH key when the caller is (re)configuring SSH auth or
	// explicitly supplies a key ID. This keeps the existing key when an unrelated
	// field is updated (the JSON zero-value "" would otherwise clear it).
	if req.SSHKeyID != "" || req.AuthType == "ssh" {
